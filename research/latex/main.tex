...
The discussed problem in this research has been tackled by many researchers over the years with varying results. in this section we intend to highlight the most important results regarding the Arabic language, the Arabic corpora and dialect classification methods that have been concluded from past research.
    ...
    \section{Methodology}
        ...
        We used preprocessing techniques that help in transforming the data to a represntation the model understands, like tokenization and segmentation which we will discuss in this section.
            ...
            While tokenizing our dataset we'll filter any non-Arabic tokens that hasn't been already filtered, such as text from other languages, emojies and other symbols. We also will remove and Arabic diacritics such as fatha, damma, kasra and so on. so the following word \RL{مُذَكِّراتِهِ} 
        ...
        \subsection{BERT}
            ...
            BERT is usually trained by predicting a masked 15\% of the input tokens that we transform to a \emph{MASK} special token, as well as giving it two sentences separated by a special \emph{SEP} token and optimizing against weather those two sentences are related or not, this naturally makes BERT a great fit for many problems such as text summarization, question answering as well as generating contextual embeddings.\cite{BERT}
    ...
    % Add brief two line intro if you think its needed
        ...
        \subsection{Datasets}
        The researchers found some difficulties to make sure that their dataset is polished. They started to remove the noises of their dataset by preprocessing their data, to assure that it will improve the accuracy and won’t ruin the machine learning classifiers task. Notable noises such as writing a nationality that conflicts with the label, non-Arabic characters, spelling mistakes, etc…
        ...
        After the success of BERT and the growing development of machine learning models and state-of-art techniques, it has become important to discover a way to process and understand the Arabic language in a much effective way. Wissam Antoun and many researchers developed AraBERT, they pre-trained BERT to be strongly focused on Arabic language and they managed to fulfill state-of-the-art performance by testing it on most Arabic NLP tasks. In their pre-training they used the \emph{Masked Language Modeling} (MLM), which it requires to understand the left and right context without any labeled data by just using a raw text.  Small percentage of input tokens are selected for replacement. Those tokens are replaced 80\% with the [MASK] token, and 20\% either a random token or the original token\cite{wissam}. Example:
        ...
        They covered 1.5 billion words from different medias and articles from diverse Arab countries. Some BERT variables are used to improve the performance, learning rate is essential for our training to achieve optimality without weakening the accuracy. the main idea is to warmup number of steps wherein the learning rate linearly increases from the minimal to the maximum and it linearly decreases over the remaining number of steps. A better learning rate will help us to decrease the amount of training hours and save our resources.
