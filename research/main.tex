...
\section{Background}
    ...
    \subsection{Dialect prediction approaches}
        ...
        Dialect recognition automatic approach is based in machine learning, where it tries to build a statistical model that learns by analysing the training data after choosing an appropriate algorithm and applying NLP techniques. The most prominent algorithms in text classification would be support vector machines (SVMs), Naïve Bayes and deep learning methods.
...
The discussed problem in this research has been tried by many researchers over the years with varying results, in this section we intend to highlight the most important results regarding the Arabic language, the Arabic corpora and dialect classification methods that has been concluded from past research.
    ...
    While the Arabic countries talks to each other in Modern Standard Arabic (\emph{MSA}), Arabic countries do not talk in MSA within themselves, they usually have a regional specific dialect. In this section, we'll detail the work made to document and breakdown dialects.
    ...
    In 2015 Shoufan and Alameri conducted a literature review, in which they summarised the advancements in NLP for dialectal Arabic\cite{shoufan_alameri_2015_natural} in the following comprehensive table. Bear in mind that the table includes more than text analysis and also includes speech analysis.
